...
description: |
  ...
  Invoke for bugs that are not obvious or hard to reproduce.
tools:
  - Read
  - Bash
  - Grep
...
- Never ship a fix without a regression test (unless purely cosmetic/UI)
- If the fix requires more than 50 lines of change, flag it as a potential design problem
- If the bug is in a dependency: document it and add a workaround wrapper, don't fork
